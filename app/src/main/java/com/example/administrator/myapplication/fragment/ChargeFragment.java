    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.fra_police_img_pic:
                ...
                Intent intent=new Intent(getActivity(), UploadChargePicActivity.class);
...
//                }
    ...
    protected void initData() {
        ...
        if (MyApplication.get().isDirectly()){
            status=StatusFactory.STEP_DIRECTLY_CHARGE;
        }else {
            status=StatusFactory.STEP_TURN_CHARGE;
        }    initExitDialog();
        initTimeFactory();
    }
   private void initExitDialog(){
    ...
    private void initTimeFactory() {
        tvTime.setText("步骤倒计时："+TimeUtil.longToTime(ConstantUtil.TIME_CHARGE));
        factory = new CountFactory(ConstantUtil.TIME_CHARGE);
    ...
    protected void setListener() {
        ...
        subscription= RxBus.getDefault().toObservable(UploadPicEvent.class).subscribe(new Action1<UploadPicEvent>() {
            ...
            public void call(UploadPicEvent uploadPicEvent) {
                if (uploadPicEvent.getType()==UploadPicEvent.TYPE_CHARGE&&uploadPicEvent.isSuccess()){
                    isUpload=true;
                }
            }
        });
