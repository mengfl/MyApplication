...
public class TurnRecordFragment  extends BaseFragment{
    @BindView(R.id.fra_turnrecord_recyclerview)
    ...
    public int layoutResourceID() {
        return R.layout.fragment_turnrecord;
    }
    ...
    private void juedge(List<Record> list){
        if (list!=null&&list.size()>0){
            ...
            for (int i=0;i<list.size();i++){
                ...
                switch (currentStep){
                    case   StatusFactory.STEP_TURN_LOGINOUT1:
                        record.setName("登入登出（第一轮）");
                        listZongHe.add(record);
                        break;
                    case   StatusFactory.STEP_TURN_LOGINOUT2:
                        record.setName("登入登出（第二轮）");
                        listZongHe.add(record);
                        break;
                    case   StatusFactory.STEP_TURN_LOGINOUT3:
                        record.setName("登入登出（第三轮）");
                        listZongHe.add(record);
                        break;
                    case   StatusFactory.STEP_TURN_POLICE:
                        record.setName("报警测试");
                        listZongHe.add(record);
                        break;
                    case   StatusFactory.STEP_TURN_SUPPLY:
                        record.setName("补发测试");
                        listZongHe.add(record);
                        break;
                    case   StatusFactory.STEP_TURN_DRIVING:
                        record.setName("行驶测试");
                        listDriving.add(record);
                        break;
                    case   StatusFactory.STEP_TURN_CHARGE:
                        record.setName("充电测试");
